...
# -*- coding: utf-8 -*-
"""
PSGS-Drug controlled ablation generation script.

Use configuration files to run A0, A1, or A2 ablation settings.
...
def mcts_for_seed(
    ...
    while time.time() <= deadline:
        ...
        try:
            ...
            print(f"[ERROR] prior handling failed seed={seed_name} iter={iter_idx}: {e}")
            if DEBUG_TRACEBACK:
                traceback.print_exc()
...
def main():
    ...
    if PRIOR_ENABLE:
        ...
        if not check_prior_service():
            print("[ERROR] Prior service unavailable. Stop.")
            sys.exit(1)
    else:
        print("  PRIOR_ENABLE=False: skip Token-Mol prior service check.")

    seeds = pick_seeds()
    if not seeds:
        print("[ERROR] No available seeds.")
        sys.exit(1)
    ...
    if not resolved_seeds:
        print("[ERROR] All seeds failed tokenization. Stop.")
        sys.exit(1)
