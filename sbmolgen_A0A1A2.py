...
------------------------------------------------------------
你的目标创新点（seeded generation）实现版：
- 从已知活性分子/核心片段 seed 出发（而不是从空状态开始）
- 生成过程中保留 seed 前缀（MCTS 只做后续扩展）
- 用 Token-Mol 双 pocket 先验给扩展 token 打分
- 用双靶点 docking + QED/SA + 多样性做 reward
- 先用原 SFG-Drug RNN rollout（不启用 CLM）

依赖：
- prior_client.py（放在 /mnt/SFG-Drug 根目录）
- utils/rdock_test_MP.py（你已经修好的版本）
- 原 SFG-Drug utils/*
"""
...
def call_prior(prefix_tokens, candidates, prior_url, pocket1_path, pocket2_path):
    ...
    pr1 = r1.get("prior", [])
    pr2 = r2.get("prior", [])
    fusion_mode = str(CONF.get("prior_fusion_mode", "mean")).lower()
    comb = _combine_dual(pr1, pr2, mode=fusion_mode)
    if comb is None:
        return [1.0 / max(1, len(candidates))] * len(candidates)
    return comb
...
def normalize_seed_prefix(seed_smiles: str) -> str:
    """把 seed 末尾的“临时生长标记”规范化为 Token-Mol/VAL 可识别的 [*]。

    设计目标（尽量“不误伤”原流程）：
    - 兼容你现有 seedmap 里末尾可能带的 "("（之前用于“生长点”但会导致 SMILES 解析失败）
    - 若出现卤素后紧跟 "("（如 "...F(" / "...Cl(" / "...Br(" / "...I("），先去掉卤素再转为 [*]
      （卤素价态=1，作为连接点会直接触发 valence error）
    - 其它情况只做“末尾 '(' -> '[*]'”的最小替换
    """
    s = (seed_smiles or "").strip()

    # A) 卤素后接 "("：去掉卤素，保留连接点到前一个原子
    #    例：...cc1F(  -> ...cc1[*]
    ...
    # C) 兜底：如果 seed 本身就以 [*] 结尾则不改；如果有孤立的 '*' 也转成 [*]
...
def sanitize_smiles_for_eval(smi: str) -> str:
    """把生成 SMILES 做最小清理，保证 RDKit / docking 入口尽量不被 [*] 这类占位符卡死。
    注意：这里只做“最小侵入”清理，不做重写生成逻辑。
    """
    if smi is None:
        return ""
    s = smi.strip()

    # 1) 去掉 Token-Mol/VAL 的 dummy 占位符（通常在末端作为生长点）
    s = s.replace("[*]", "")

    # 2) 有些版本 vocab 里会单独出现 '*'，也移除
    s = s.replace("*", "")
...
# =========================================================
#  配置读取（固定支持 mcts.hours / hours_per_seed）
# =========================================================
def load_conf() -> Dict[str, Any]:
    """
    优先级：
      1) 命令行参数 --cfg
      2) 环境变量 SFGDRUG_CFG_PATH
      3) 默认路径列表
    """
    import sys
    print("DEBUG: load_conf started")

    # 1) 检查命令行 --cfg
    for i, arg in enumerate(sys.argv):
        if arg == "--cfg" and i + 1 < len(sys.argv):
            cfg_path = sys.argv[i + 1]
            print(f"DEBUG: found --cfg = {cfg_path}")
            if os.path.exists(cfg_path):
                with open(cfg_path, "r", encoding="utf-8") as f:
                    data = yaml.load(f, Loader=yaml.SafeLoader) or {}
                data["_cfg_path"] = cfg_path
                print("DEBUG: config loaded from command line")
                return data
            else:
    ...
    if env_cfg:
        print(f"DEBUG: using env SFGDRUG_CFG_PATH = {env_cfg}")
    ...
    # 3) 默认路径
    candidates = [
        "/mnt/SFG-Drug/example_ligand_design/setting_prior_contactfrag_run.yaml",
        os.path.join(os.getcwd(), "setting_prior_contactfrag_run.yaml"),
        "setting_prior_contactfrag_run.yaml",
    ]
    for p in candidates:
    ...
    raise FileNotFoundError("未找到 seed 配置文件。请设置环境变量 SFGDRUG_CFG_PATH 或提供 --cfg 参数。")
...
PRIOR_ENABLE = bool(CONF.get("prior_enable", True))
...
# hours 修复：优先 hours_per_seed，其次 mcts.hours，再次顶层 hours
...
W_PAINS = float(RCONF.get("w_pains_penalty", 0.00))  # 当前版本未实现 PAINS，保留接口
...
SEED_SMILES_OVERRIDE = CONF.get("seed_smiles_override", []) or []  # 推荐使用这个字段（完整有效SMILES）
...
# 新增：从配置中读取 fragment prefix 相关设置
SEED_FRAGMENT_PREFIXES = CONF.get("seed_prefixes_override", []) or []
# 兼容：直接吃 extract_contact_seed_fragments_v2.py 的输出字段（不需要改你原来的导出脚本）
# - seed_scaffolds_no_star_override: 直接可作为 prefix（最推荐，去掉 '*' 之后的可分词版本）
# - seed_scaffolds_override: 含 '*' 的片段（如果你的 vocab 里有 '*'，也可用；否则会在分词阶段被 skip）
...
SEED_FRAGMENT_MODE = CONF.get("seed_fragment_mode", "prefer")        # 默认 prefer
...
def build_vocab_and_model():
    ...
    zinc_candidates = [
        ...
        "/mnt/SFG-Drug/data/250k_rndm_zinc_drugs_clean.smi",
    ...
    if zinc_path is None:
        raise FileNotFoundError("未找到 ZINC 词表文件，请设置 SFGDRUG_ZINC_PATH")
...
def greedy_tokenize_by_val(smiles: str, val: List[str]) -> Optional[List[str]]:
    """
    按 VAL 词表做最长匹配分词。
    兼容 SFG-Drug 的多字符 token（如 [C@H], Cl, Br 等）。
    """
    if not smiles:
        return None

    # 词表按长度降序，避免把 [C@H] 拆成 '[' 'C' '@' ...
...
def normalize_seed_string(seed: str) -> List[str]:
    """
    对带 dummy '*' 的 seed scaffold 做若干可运行化尝试。
    注意：这是工程折中方案，保证“能跑起来”优先。
    """
    cands = []
    s = seed.strip()
...
def pick_seeds() -> List[Tuple[str, str]]:
    """
    Return [(seed_name, seed_smiles_or_prefix), ...].
    If seed_enable is false, force empty-state generation.
    """
    if not SEED_ENABLE:
        print("[SEED_OFF] seed_enable=False -> using only seed_empty")
        return [("seed_empty", "")]
...
def _is_balanced_prefix_smiles(s: str) -> bool:
    """
    轻量前缀合法性检查（不要求完全闭合，只要求不出现明显语法残缺）：
    - 括号/方括号不能出现“提前闭合”
    - 不能以反斜杠/斜杠/双键/三键等键符号结尾（会显著提高无效率）
    - 不能以 '%' 开头的环号未完整（例如末尾是 '%' 或 '%1'）
    """
    if not s:
        return True
...
def _is_good_growth_endpoint_token(tok: str) -> bool:
    """
    经验规则：为了让“前缀作为生长点”更稳定，尽量让末尾是一个原子/闭括号/环号。
    """
    if tok in ('=', '#', '/', '\\', '%'):
        return False
    # 原子 token（按本项目 VAL 常见集合）
...
def _sanitize_seed_prefix(seed_raw: str, val: List[str], max_len_tokens: int = 120) -> Tuple[Optional[List[str]], str]:
    """
    仅用于“seed 前缀模式”：在不改动其它逻辑的前提下，把 seed_raw 裁剪到
    1) 可被 val 分词
    2) 前缀轻量语法检查通过
    3) 末尾 token 适合作为生长点
    返回 (tokens or None, used_prefix_str)
    """
    ...
    # 逐步向左裁剪（字符级），直到满足分词 + 轻量检查 + 末尾 token 合格
    # 这是“只改输入”的关键：保证 prefix 不会把后续生成推到大量 RDKit 解析失败区。
...
def resolve_seed_to_tokens(seed_name: str, seed_raw: str, val: List[str]) -> Tuple[Optional[List[str]], str]:
    """
    把 seed（完整分子 / scaffold / 未闭合前缀）转成可用于 SFG-Drug 的 token 序列
    返回: (tokens or None, chosen_seed_string)
    """
    if not seed_raw:
        return [], ""
    ...
    # 片段/前缀模式：只在“输入端”做安全裁剪与分词，避免大量 RDKit 解析失败
    ...
    # 最后尝试：兼容部分项目版本的 make_input_smile
    try:
...
def check_prior_service() -> bool:
    ...
    try:
        if not PRIOR_URL:
            print("❌ prior_url 未配置（PRIOR_URL=None）")
            return False
        if not P1 or not P2:
            print("❌ prior_protein_path_1 / prior_protein_path_2 未配置")
            return False
        ...
        try:
            ...
            print("❌ 先验服务返回内容不可解析为数值")
            return False

        print("✅ 先验服务检查通过")
        print(f"   检查用先验范围: {pmin:.6f} - {pmax:.6f}")
        return True
...
def docking_to_reward(scores) -> Tuple[float, float, float]:
    ...
    # --- 双靶点 docking reward 组合策略 ---
    # 说明：
    #   delta < 0 表示比 base 更好（更负）；delta > 0 表示更差
    #   combined = delta1 + delta2 反映“总体改进”
    #   worst = max(delta1, delta2) 反映“短板”（更差那侧）
    combined = delta1 + delta2
    worst = max(delta1, delta2)
    imbalance = abs(delta1 - delta2)

    def _sat(x: float, k: float) -> float:
        # 平滑饱和：x<0（变好）-> 正奖励；x>0（变差）-> 负奖励
    ...
    if mode in ("baseline_like", "sum_heavy", "sum-heavy"):
        ...
        # 硬门槛：两边都必须达到阈值，否则直接低分（节省 docking 预算时很有用）
...
def mcts_for_seed(
    ...
    """
    在单个 seed 上运行一段时间（HOURS_PER_SEED）
    返回当前 seed 的局部统计 seed_stat
    """
    ...
    while time.time() <= deadline:
        ...
        if node is None:
            continue
        # 深度保护：避免在终止符/超长处反复空转。
        # 策略：遇到换行token或长度>=82时，回退一个token（回到父节点）再尝试扩展。
        ...
        try:
            ...
            print(f"[WARN] expanded_node异常: {e}")
            if DEBUG_TRACEBACK:
                traceback.print_exc()
            cur = node
            while cur is not None:
        ...
        # -------- Token-Mol dual-pocket prior / uniform prior for ablation --------
        prefix = ["<|beginoftext|>", "<|mask:0|>", "<|mask:0|>"] + [
            t for t in state.position if t != "&"
        ]
        ...
        try:
            if PRIOR_ENABLE:
                ...
                if iter_idx <= 3:
                    print(
                        f"[PRIOR_ON] iter={iter_idx} seed={seed_name} "
                        f"fusion={CONF.get('prior_fusion_mode', 'mean')} "
                        f"prefix_tail={prefix[-12:]} candidates_head={candidates[:12]}",
                        flush=True,
                    )
                ...
                if not isinstance(priors, (list, tuple)) or len(priors) != len(candidates):
                    raise RuntimeError(
                        f"prior len mismatch: "
                        f"{len(priors) if hasattr(priors, '__len__') else 'NA'} "
                        f"vs {len(candidates)}"
                    )
                ...
                if iter_idx <= 10:
                    ...
                    try:
                        print(
                            f"[prior_stat] iter={iter_idx} "
                            f"min={min(pv):.6g} max={max(pv):.6g} "
                            f"uniq={len(set([round(float(x), 6) for x in pv]))}",
                            flush=True,
                        )
                    except Exception:
                        print(
                            f"[prior_stat] iter={iter_idx} min={min(pv)} max={max(pv)}",
                            flush=True,
                        )
                    ...
                    print(
                        "[prior_top5] "
                        + ", ".join([f"{t}:{prior_map.get(t, 0.0):.4g}" for t in top5]),
                        flush=True,
                    )

            else:
                # A0: no Token-Mol prior. Use uniform prior over current candidates.
                uniform_p = 1.0 / max(1, len(candidates))
                prior_map = {tok: uniform_p for tok in candidates}

                if iter_idx <= 10:
                    print(
                        f"[PRIOR_OFF] iter={iter_idx} seed={seed_name} "
                        f"using uniform prior; candidates_head={candidates[:12]}",
                        flush=True,
                    )

        except Exception as e:
            print(f"❌ prior handling failed seed={seed_name} iter={iter_idx}: {e}")
            if DEBUG_TRACEBACK:
                traceback.print_exc()
        ...
        # ----------------- 多样化的 prior-guided child sampling -----------------
        # 过去版本直接取 prior top-N，容易塌缩导致重复率高。这里改为：
        #   1) temperature 拉平分布
        #   2) top_k / top_p 截断
        #   3) Dirichlet noise (可选) 增强探索
        #   4) 无放回采样 allow 个 child
        ...
        try:
            ...
            print(f"[WARN] check_node_type异常: {e}")
            if DEBUG_TRACEBACK:
                traceback.print_exc()
            cur = node
            while cur is not None:
        ...
        # -------- 便宜过滤前置 + docking（使用修好的 rdock_test_MP）--------
        # 注意：有些工程版本 check_node_type 里已经 dock 过；但为统一稳定性，这里重新用 rdock_test_MP 计算真实双靶分数
        # 如果你确认 check_node_type 内部不会返回伪值，也可以切换为“仅校验/回退模式”
        candidates_for_dock = []
        ...
        for smi, meta in candidates_for_dock:
            ...
            if not smi_eval:
                continue
            # 如果已经见过该分子指纹，则默认跳过 docking（节省时间 + 提升 unique）
            ...
            try:
                scores = vinadock_score(smi_eval)  # 关键：用你修好的版本
            except Exception as e:
                print(f"[WARN] vinadock_score异常: {smi} -> {e}")
                if DEBUG_TRACEBACK:
                    traceback.print_exc()
                continue
            ...
            # 奖励回传给当前扩展对应 child（粗粒度近似：用本轮 nodeadded_all 的 token）
            # 如果无法准确映射，至少回传到当前 node
            assigned = False
...
#  主程序
# =========================================================
def print_ablation_audit(stage: str = "startup"):
    print(f"\n========== ABLATION AUDIT: {stage} ==========")
    print(f"cfg_path                  = {CONF.get('_cfg_path', 'unknown')}")
    print(f"target                    = {TARGET}")
    print(f"trial                     = {TRIAL}")
    print(f"target_total_mols         = {TARGET_TOTAL_MOLS}")
    print(f"hours_per_seed            = {HOURS_PER_SEED}")

    print("\n[Switches]")
    print(f"PRIOR_ENABLE              = {PRIOR_ENABLE}")
    print(f"SEED_ENABLE               = {SEED_ENABLE}")
    print(f"prior_fusion_mode         = {CONF.get('prior_fusion_mode', 'mean')}")

    print("\n[Prior]")
    print(f"prior_url                 = {PRIOR_URL}")
    print(f"prior_protein_path_1      = {P1}")
    print(f"prior_protein_path_2      = {P2}")
    print(f"prior_temperature         = {CONF.get('prior_temperature', None)}")
    print(f"prior_top_k               = {CONF.get('prior_top_k', None)}")
    print(f"prior_top_p               = {CONF.get('prior_top_p', None)}")
    print(f"prior_dirichlet_alpha     = {CONF.get('prior_dirichlet_alpha', None)}")
    print(f"prior_dirichlet_eps       = {CONF.get('prior_dirichlet_eps', None)}")

    print("\n[Seed]")
    print(f"seed_fragment_mode        = {SEED_FRAGMENT_MODE}")
    print(f"seed_fragment_prefer_sp3  = {SEED_FRAGMENT_PREFER_SP3}")
    print(f"seed_max_num              = {SEED_MAX_NUM}")
    print(f"n_seed_prefixes           = {len(SEED_FRAGMENT_PREFIXES)}")

    if SEED_FRAGMENT_PREFIXES:
        for i, s in enumerate(SEED_FRAGMENT_PREFIXES[:10], 1):
            print(f"  seed_prefix_{i}          = {s}")

    print("\n[MCTS]")
    print(f"C_VAL                     = {C_VAL}")
    print(f"EXPANDED_K                = {EXPANDED_K}")
    print(f"SIM_NUM                   = {SIM_NUM}")

    print("\n[Prefilter]")
    print(f"PF_QED_MIN                = {PF_QED_MIN}")
    print(f"PF_SA_MAX                 = {PF_SA_MAX}")
    print(f"PF_LIP_MAX_VIOL           = {PF_LIP_MAX_VIOL}")
    print(f"DOCK_MAX_PER_ROLLOUT      = {DOCK_MAX_PER_ROLLOUT}")

    print("\n[Reward]")
    print(f"W_DOCK                    = {W_DOCK}")
    print(f"W_QED                     = {W_QED}")
    print(f"W_SA                      = {W_SA}")
    print(f"W_LIP                     = {W_LIP}")
    print("============================================\n")
...
def main():
    ...
    print_ablation_audit("before generation")

    if PRIOR_ENABLE:
        print("🔍 PRIOR_ENABLE=True: checking Token-Mol prior service...")
        if not check_prior_service():
            print("❌ Prior service unavailable. Stop.")
            sys.exit(1)
    else:
        print("ℹ PRIOR_ENABLE=False: skip Token-Mol prior service check.")

    seeds = pick_seeds()
    if not seeds:
        print("❌ No available seeds.")
        sys.exit(1)

    print("\n========== SEED AUDIT ==========")
    print(f"SEED_ENABLE = {SEED_ENABLE}")
    print(f"number of picked seeds = {len(seeds)}")
    for i, (seed_name, seed_raw) in enumerate(seeds, 1):
        print(f"[seed {i}] name={seed_name} raw={seed_raw}")
    print("================================\n")
    ...
    for seed_name, seed_raw in seeds:
        ...
        if toks is None:
            print(f"[seed][SKIP] {seed_name} tokenize failed: {seed_raw}")
            seed_rows.append({
                "seed_name": seed_name,
                "seed_raw": seed_raw,
                "seed_used": used_smi,
    ...
    if not resolved_seeds:
        print("❌ All seeds failed tokenization. Stop.")
        sys.exit(1)
    ...
    print("🚀 Start MCTS-GRU generation")
    print(f"    PRIOR_ENABLE={PRIOR_ENABLE}, SEED_ENABLE={SEED_ENABLE}")
