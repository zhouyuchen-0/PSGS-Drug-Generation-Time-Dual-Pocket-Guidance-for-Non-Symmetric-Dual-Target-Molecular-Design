...
def call_prior(prefix_tokens, candidates, prior_url, pocket1_path, pocket2_path, prior_mode="dual"):
    """
    prior_mode:
      - dual:     双口袋融合
      - single_1: 只用 pocket1
    ...
    if prior_mode == "single_1":
        r1 = _post_json(prior_url, {**payload_base, "protein_path": pocket1_path})
        if r1.get("status") != "ok":
            return [1.0 / max(1, len(candidates))] * len(candidates)
        pr1 = r1.get("prior", [])
        if not pr1 or len(pr1) != len(candidates):
            return [1.0 / max(1, len(candidates))] * len(candidates)
        s = float(sum(pr1))
        if s <= 0:
            return [1.0 / max(1, len(candidates))] * len(candidates)
        return [float(x) / s for x in pr1]

    if prior_mode == "single_2":
        r2 = _post_json(prior_url, {**payload_base, "protein_path": pocket2_path})
        if r2.get("status") != "ok":
            return [1.0 / max(1, len(candidates))] * len(candidates)
        pr2 = r2.get("prior", [])
        if not pr2 or len(pr2) != len(candidates):
            return [1.0 / max(1, len(candidates))] * len(candidates)
        s = float(sum(pr2))
        if s <= 0:
            return [1.0 / max(1, len(candidates))] * len(candidates)
        return [float(x) / s for x in pr2]
...
PRIOR_MODE = str(CONF.get("prior_mode", "dual")).strip().lower()
...
def pick_seeds() -> List[Tuple[str, str]]:
    """
    返回 [(seed_name, seed_smiles_or_prefix), ...]
    优先级（默认）：
      seed_smiles_override > seed_scaffolds_override > seed_drugs
    当 seed_fragment_mode=prefer/only 时，会优先或仅使用 seed_fragment_prefixes（若为空则用内置示例前缀）
    """
    if not SEED_ENABLE:
        return [("seed_empty", "")]
    ...
    # 输入排序：优先选择“更像 sp3 链端”的前缀作为生长点（减少括号/环闭合导致的 RDKit 失败）
    # 说明：这是启发式输入排序，不改变采样/搜索主逻辑；仅影响种子枚举顺序。
    if SEED_FRAGMENT_PREFER_SP3 and fragment_pool:
        def _sp3_score(s: str) -> int:
            ...
            if ss.endswith(tuple("CNOSPFI")):
                return 6
            if ss.endswith(tuple("cnos")):  # 芳香结尾：更容易引入 kekulize/环闭合问题
                return 1
...
def check_prior_service() -> bool:
    ...
    try:
        ...
        if PRIOR_MODE == "single_1":
            if not P1:
                print("❌ single_1 模式下 prior_protein_path_1 未配置")
                return False
        elif PRIOR_MODE == "single_2":
            if not P2:
                print("❌ single_2 模式下 prior_protein_path_2 未配置")
                return False
        else:
            if not P1 or not P2:
                print("❌ dual 模式下 prior_protein_path_1 / prior_protein_path_2 未完整配置")
                return False
        ...
        priors = call_prior(test_prefix, filtered, PRIOR_URL, P1, P2, prior_mode=PRIOR_MODE)
...
def mcts_for_seed(
    ...
    while time.time() <= deadline:
        ...
        try:
            ...
            priors = call_prior(prefix, candidates, PRIOR_URL, P1, P2, prior_mode=PRIOR_MODE)
            ...
            print(f"❌ 先验服务调用失败（seed={seed_name} iter={iter_idx}）: {e}")
            if DEBUG_TRACEBACK:
                traceback.print_exc()
            # 必须依赖 prior，直接给负反馈并继续
            cur = node
            while cur is not None:
                cur.update(-1.0)
                cur = cur.parentNode
...
def main():
    ...
    for seed_name, seed_raw in seeds:
        ...
        if toks is None:
            print(f"[seed][SKIP] {seed_name} 无法分词: {seed_raw}")
            seed_rows.append({
                "seed_name": seed_name,
                "seed_raw": seed_raw,
                "seed_used": used_smi,
    ...
    print("🚀 开始 Seeded MCTS + Token-Mol 双 pocket 先验生成")
