...
# -*- coding: utf-8 -*-
"""
Analyze B1/B2 single-pocket controls and the contact-guided final-generation set.

The script quantifies target-order sensitivity and contextualizes the formal
contact-guided set under the same downstream docking protocol.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze B1/B2 controls and contact-guided set.")
    parser.add_argument("--b1_csv", required=True, help="B1 3FAP-only post-screened CSV.")
    parser.add_argument("--b2_csv", required=True, help="B2 7PQV-only post-screened CSV.")
...
def main() -> None:
    ...
    datasets = {
        "B1_3FAP_first": load_and_standardize(Path(args.b1_csv), "B1_3FAP_first"),
        "B2_7PQV_first": load_and_standardize(Path(args.b2_csv), "B2_7PQV_first"),
        "Contact_guided_final": load_and_standardize(Path(args.contact_csv), "Contact_guided_final"),
    }

    full_rows = [summarize_method(df, name) for name, df in datasets.items()]
    full_df = pd.DataFrame(full_rows)
    full_df.to_csv(out_dir / "B1_B2_contact_full_summary.csv", index=False)

    topk_rows = []
    for name, df in datasets.items():
        for k in args.topk:
            topk_rows.append(topk_summary(df, name, k))
    pd.DataFrame(topk_rows).to_csv(out_dir / "B1_B2_contact_topk_summary.csv", index=False)
